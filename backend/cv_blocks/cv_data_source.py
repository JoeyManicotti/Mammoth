...
class CVDataSourceBlock(BaseBlock):
    ...
    def configure(self, **kwargs):
        """Configure CV data source parameters"""
    ...
    def validate_config(self):
        ...
        if source == 'video' and 'video_path' not in self.config:
            errors.append("video_path required for video source")

        if source == 'images' and 'image_dir' not in self.config:
            errors.append("image_dir required for images source")

        if source == 'benchmark' and 'dataset' not in self.config:
            errors.append("dataset required for benchmark source")
    ...
    def get_schema(self):
        ...
        return {
            'type': 'cv-data-source',
            'inputs': {},
            'outputs': {'frames': 'List[ndarray]', 'init_bbox': 'List[int]'}
        }
